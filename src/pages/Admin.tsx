...
// --- AUTH GATE COMPONENT (LOGIN FORM) ---
const AuthGate = ({ onLoginSuccess }: { onLoginSuccess: () => void }) => {
    ...
    const handleLogin = (e: React.FormEvent) => {
        ...
        if (authEmail === AUTH_CREDENTIALS.EMAIL && authPassword === AUTH_CREDENTIALS.PASSWORD) {
            onLoginSuccess();
            toast({
    ...
    return (
        <Layout>
            <section className="min-h-screen flex items-center justify-center bg-primary/10">
                <Card className="w-full max-w-md shadow-soft">
                    <CardHeader className="text-center">
                        <Lock size={48} className="text-primary mx-auto mb-2" />
                        <CardTitle className="text-3xl font-heading font-bold">Admin Access</CardTitle>
                        <CardDescription>Please log in to manage the dashboard.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleLogin} className="space-y-6">
                            ...
                            <Button type="submit" className="w-full bg-primary text-primary-foreground hover:bg-primary-light rounded-full">
...
const Admin = () => {
    ...
    const [feedbacks, setFeedbacks] = useState<any[]>([]); // Use any[] for dynamic data
    const [inquiries, setInquiries] = useState<any[]>([]); // Use any[] for dynamic data
    const [loadingFeedbacks, setLoadingFeedbacks] = useState(true);
    const [loadingInquiries, setLoadingInquiries] = useState(true);
    const [filterType, setFilterType] = useState('all');
    const [inquiryFilter, setInquiryFilter] = useState('all');

    // Callback function to handle successful login from AuthGate
    ...
    useEffect(() => {
        ...
        const fetchFeedbacks = async () => {
            // ... (your existing fetchFeedbacks logic)
            try {
        ...
        const fetchInquiries = async () => {
            // ... (your existing fetchInquiries logic)
            try {
        ...
        fetchFeedbacks();
        fetchInquiries();
    }, [isLoggedIn, toast]); // Dependency array includes isLoggedIn
    ...
    const handleStatusChange = async (uniqueId: string, newStatus: string) => {
        ...
        try {
            await fetch(SCRIPT_URL_INQUIRIES, {
                method: 'POST',
                mode: 'no-cors',
                headers: {
    ...
    const handlePhotoUpload = async (e: React.FormEvent) => {
        e.preventDefault();
        // ... (your existing handlePhotoUpload logic)
